...
- **E2E:** `npm run e2e` with the Vite dev server running (`cd frontend && npm run dev` → [http://localhost:5173](http://localhost:5173)). Playwright `baseURL` defaults to `http://localhost:5173`; set **`FRONTEND_URL`** to hit production (e.g. Vercel) instead.

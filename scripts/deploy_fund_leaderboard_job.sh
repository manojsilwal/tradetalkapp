#!/usr/bin/env bash
# deploy_fund_leaderboard_job.sh — Cloud Run Job + weekly Cloud Scheduler trigger
#
# Full 13F ingest runs as a dedicated Cloud Run Job (not fire-and-forget on the API)
# so it survives scale-to-zero and has a 1-hour task timeout.
#
# Usage:
#   bash scripts/deploy_fund_leaderboard_job.sh
#   bash scripts/deploy_fund_leaderboard_job.sh --execute
#   bash scripts/deploy_fund_leaderboard_job.sh --skip-build

set -euo pipefail

PROJECT_ID="${GCP_PROJECT_ID:-tradetalkapp-492904}"
REGION="${GCP_REGION:-us-central1}"
JOB_NAME="fund-leaderboard-ingest"
IMAGE="gcr.io/${PROJECT_ID}/sp500-ingest:latest"
SA_EMAIL="tradetalk-etl@${PROJECT_ID}.iam.gserviceaccount.com"
SCHEDULER_NAME="precompute-fund-leaderboard"
CRON="0 6 * * 1"
EXECUTE=0
SKIP_BUILD=0
...
echo "=== Deploy fund leaderboard ingest job + scheduler ==="
echo "Project:  $PROJECT_ID"
echo "Job:      $JOB_NAME"
echo "Schedule: $CRON (UTC, Mondays)"
echo ""
...
gcloud run jobs deploy "$JOB_NAME" \
  ...
  --max-retries 1 \
  --command bash \
  --args scripts/run_fund_leaderboard_job.sh \
  --set-env-vars "MCP_DATA_BACKEND=bigquery,GCP_PROJECT_ID=${PROJECT_ID}" \
  --quiet

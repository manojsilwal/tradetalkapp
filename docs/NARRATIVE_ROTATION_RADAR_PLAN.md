...
**Status:** Implemented and complete (NR-1..NR-9 + §7 integration + §21 MVP checklist). Backend `backend/narrative_radar/` (taxonomy, features, scoring with all 8 signal families, lifecycle classifier, signals orchestrator, institutional 13F aggregation, alerts, backtests, evidence timeline, store, ledger, engine) + connectors `backend/connectors/etf_filings.py` (N-1A/S-1) and `backend/connectors/etf_flows.py`. API `backend/routers/narrative_radar.py` (`/themes`, `/overview`, `/themes/{slug}`, `/themes/{slug}/timeline`, `/themes/{slug}/stocks`, `/alerts`, `/backtests`, `/refresh`, `/status`) behind `NARRATIVE_RADAR_ENABLE`. Cron `POST /knowledge/narrative-radar-run` + APScheduler daily job (00:50 UTC) in `daily_pipeline.py`. CORAL `handoff_theme_phase` event emitted per scan for nightly dreaming. Per-family **data-freshness** + **confidence** + **explanation JSON** + **compliance disclaimer** on every surface (§21). Frontend `frontend/src/NarrativeRadarUI.jsx` at `/narrative-radar` (lifecycle cards, heatmap, alerts panel, backtest KPI, detail drawer with signal families + evidence timeline + data freshness). Tests `backend/tests/test_narrative_radar.py` (36 offline). Network-heavy families (productization, ETF flows, narrative/social, reality) are flag-gated OFF by default so the default scan stays MVP-fast; institutional 13F aggregation reads the local fund-leaderboard DB and degrades to unavailable when empty.

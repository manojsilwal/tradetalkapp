...
def cached_tool_call(tool_name: str, args: Any, fn: Callable[[], Any]) -> Any:
    ...
    with _lock:
        ...
        # Cache miss — compute (outside of lock to allow concurrent backend calls)
        _misses += 1

    # Call the backend function outside the lock to avoid holding the lock during slow network/db calls
    result = fn()

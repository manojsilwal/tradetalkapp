...
Pipeline:
  1. LLM (OpenRouter) breaks the lesson topic into 8 visual scenes (4 seconds each by default)
...
Scene JSON is produced via OpenRouter. MP4 clips use Google Veo (Gemini API). If Veo fails for a
scene and VIDEO_VEO_OPENROUTER_FALLBACK=1 (default), the same lesson continues with a text slide
generated by OpenRouter (Qwen) — OpenRouter cannot render MP4 video, only substitute copy.
...
# When Veo cannot produce an MP4 for a scene, generate a text slide via OpenRouter (Qwen).
...
async def _text_fallback_playlist_entry(
    ...
    """
    Text-only slide when Veo MP4 is unavailable. Uses OpenRouter (Qwen) when configured,
    otherwise llm_client rule-based templates.
    """
